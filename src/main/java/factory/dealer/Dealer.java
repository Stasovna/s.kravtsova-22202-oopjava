...
public class Dealer implements Runnable {
    ...
    @Getter
    private final int id = SEQ.getAndIncrement();//потокобезопасно создаем айдишики для диллеров
    ...
    public void run() {
        while (!Thread.currentThread().isInterrupted()) {
            ...
            synchronized (controllerLock) {//если не занят
                controllerLock.notify();//уведомляем того который ждет
            }
